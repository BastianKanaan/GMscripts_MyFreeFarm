...
// Do not edit ********************************************************************************************************
    ...
    // Important constants
    const COUNTRY="DK"; // The country ISO-code (2 digits)
    const LANGUAGE="dk"; // The language ISO-code (2 digits)        
...
// Take the following from the game ***********************************************************************************
    if(undefined===text[LANGUAGE]){
        ...
        text[LANGUAGE]["msgMarketsaleContent"]="(.*) har ved markedspladsen købt\\s*(\\d+)x (.*?) af dig<br>\\s*for(.*?) kD\\."; 
        ...
        text[LANGUAGE]["msgContractsaleContent"]="(.*) har underskrevet en kontrakt fra dig!<br><br>\\s*Følgende produkter blev solgt:<br>([\\S\\s]*)\\s*<br>\\s*Kontraktsummen på (.*?) kD blev indsat på din konto\\."; 
        ...
        text[LANGUAGE]["msgSubjectFriend"]="(.+) wants to add you as friend";
