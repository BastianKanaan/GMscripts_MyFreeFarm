...
const neededVersionBerater="2.5.9";
...
try{
    ...
    if(divChooseEndTimeCurr){
        ...
        switch(fzType){
        case 1:case 4:case "megafield":{ // fields, foodworld, megafield
...
try{
    ...
    switch (zoneTyp){
        case 1:
            ...
            for(var v=1;v<=120;v++){
                // GM_log(v);
                if(("v"==unsafeWindow.garten_kategorie[v]||"hr"==unsafeWindow.garten_kategorie[v]) && (unsafeWindow.garten_zeit[v]!="0") && (parseInt(unsafeWindow.garten_zeit[v],10)<NowServer)){
                    cropable=true;
                    break;
                }
            }
...
try{
    if(bot.checkRun("autoFarm",runId,true)){
        ...
        if(unsafeWindow.mode!="1"){ 
            ...
            if("v"==unsafeWindow.garten_kategorie[v]||"hr"==unsafeWindow.garten_kategorie[v]){
...
try{
    ...
    if(bot.checkRun("autoFarmPlantInit",runId)){
        ...
        if(unsafeWindow.racksort&&(unsafeWindow.racksort>5)){
            ...
            var NowServer=unsafeWindow.Zeit.Server;
            for(var v=1;v<=120;v++){
                if("v"==unsafeWindow.garten_kategorie[v]||"hr"==unsafeWindow.garten_kategorie[v]){ // only plants
                    var z=parseInt(unsafeWindow.garten_zeit[v],10);
                    if(z>0){
            ...
            if(rackitemNr==null){
                ...
                if(rackitemNr!=null){
                    ...
                    for(var v=0;v<unsafeData.prodTyp[0].length;v++){ // all
                        if((1==unsafeData.PRODUCT2BUILDING[0][v])&&(calcEmptyProductPositions(v,handled.zoneNrF)>0)&&unsafeData.prodStock[0][v]&&unsafeData.prodStock[0][v]>0){
                            plants=true;
                            break;
                        }
                    }
...
function autoFarmFactoryOil(runId,step){
    ...
    if(bot.checkRun("autoFarmFactoryOil",runId)){
        bot.setAction("autoFarmFactoryOil ("+step+")");
        var action=null,listeningEvent=null;
        switch(step){
        case 1:{ // init
...
function autoFarmFactoryKnitting(runId,step){
    ...
    if(bot.checkRun("autoFarmFactoryKnitting",runId)){
        bot.setAction("autoFarmFactoryKnitting ("+step+")");
        var action=null,listeningEvent=null;
        switch(step){
        case 1:{ // init
            // TODO check required products earlier (recalcQueue)
...
function autoMegafield(runId,step){
    ...
    if(bot.checkRun("autoMegafield",runId)){
        ...
        case 2:{ // switch action
            if(("object"==typeof unsafeWindow.megafield_data.tour)&&(!unsafeWindow.megafield_data.tour.remain)){
                unsafeWindow.megafield_data.tour=0;
            }
            if((!unsafeWindow.megafield_data.tour)&&(help=unsafeData.readyZone[handled.zoneNrL])&&("r"==help[1])&&((PRODSTOP!=zoneList[handled.zoneNrL][0][0])||(!settings.get("account","disableCropFields")))){
                // crop
                autoMegafield(runId,step+1);
        ...
        case 3:{ // crop vehicle
            ...
            }else if(unsafeWindow.megafield_vehicle_id&&(unsafeWindow.megafield_tour_type=="harvest")){
        ...
        case 7:{ // Plant
            ...
            }else if(!unsafeData.gameLocation.check("megafield")){
                // open megafield
                GM_logInfo("autoMegafield","runId="+runId+" step="+step,"zoneNrF="+handled.zoneNrF+" zoneNrL="+handled.zoneNrL,getText("automat_automatMegafield")+": "+getText("automat_changingToX").replace("%1%",getText("megafield")));
                step--;
                listeningEvent="gameOpenMegafield";
